...
//        return courses;
//    }
    public ArrayList<Category> get4CategoryRand() {
        ArrayList<Category> list = new ArrayList<>();
        String sql = "	WITH RandomCategories AS (\n"
                + "    SELECT TOP 4 \n"
                + "        ct.CourseCateroryId,\n"
                + "        ct.[CategoryName]\n"
                + "    FROM \n"
                + "        [dbo].[CourseCategory] ct\n"
                + "    ORDER BY \n"
                + "        NEWID()\n"
                + "),\n"
                + "CourseCount AS (\n"
                ...
                + "    RandomCategories rc\n"
                + "LEFT JOIN \n"
                + "    CourseCount cc ON rc.CourseCateroryId = cc.CourseCateroryId;";
        ...
        try {
            ...
            while (rs.next()) {
                String cate_id = rs.getString(1);
                String cate_name = rs.getString(2);
                int course_count = rs.getInt(3);
                list.add(new Category(cate_id, cate_name, course_count));
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    public static void main(String[] args) throws ClassNotFoundException, SQLException {
        ...
        System.out.println(dao.get4CategoryRand());
    }
}
