...
public class SignUpServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (!email.matches("[a-zA-Z0-9]+@([a-zA-Z]+.){1,2}[a-zA-Z]+")) {
            session.setAttribute("error", "Please Enter Email matches with fomat A@gmail.com");
            request.setAttribute("fullname", fullname);
            request.setAttribute("password", password);
            response.sendRedirect("index.jsp?openModal_signup=true");
            ...
            if (accountFoundByNamePass != null) {
                ...
                session.setAttribute("error1", "Account is exit !!!");
                response.sendRedirect("index.jsp?openModal_signup=true");
                //request.getRequestDispatcher("index.jsp").forward(request, response);
                return;
            } else {
                ...
                Account account_to_check = new Account(password, email);
                Account account_to_login = accountDAO.findByEmailAndPass(account_to_check);
                session.setAttribute("account", account_to_login);
